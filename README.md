# LLM Agent系统（ReAct + RAG）

一个基于大语言模型的Agent系统，实现ReAct推理流程、工具调用与RAG检索增强，支持多轮对话，并对Agent执行过程进行约束与优化，提升稳定性。

------------------------------------------------------------------------

## 🚀 项目特点

-   基于 ReAct 思想的Agent执行流程（Thought / Action / Observation）
-   支持 RAG（检索增强生成），减少模型幻觉
-   实现工具调用控制机制（限制调用次数，避免循环执行）
-   支持多轮对话与上下文管理
-   提供三种使用方式：API / CLI / 前端交互

------------------------------------------------------------------------

## 🧠 系统架构

LLM + Agent（ReAct） + RAG（Qdrant） + Memory（Neo4j）

------------------------------------------------------------------------

## 📂 项目结构

    llm-agent-system/
    │
    ├── app/                  # FastAPI入口
    ├── agent/                # Agent逻辑（ReAct）
    ├── rag/                  # RAG模块（向量检索）
    ├── memory/               # 记忆模块（Neo4j）
    ├── context/              # 上下文构建
    ├── frontend/             # 前端（Streamlit）
    ├── scripts/              # CLI调试
    │
    ├── requirements.txt
    ├── .env.example
    └── README.md

------------------------------------------------------------------------

## ⚙️ 环境配置

``` bash
pip install -r requirements.txt
cp .env.example .env
```

------------------------------------------------------------------------

## ▶️ 运行方式

### 1️⃣ 启动后端服务

``` bash
uvicorn app.main:app --reload
```

接口：

POST /chat

请求示例：

``` json
{
  "query": "你的问题"
}
```


------------------------------------------------------------------------

### 2️⃣ 启动前端

``` bash
streamlit run frontend/app.py
```

启动后可在浏览器进行交互式问答。

------------------------------------------------------------------------

### 3️⃣ CLI 调试模式

``` bash
python scripts/cli_test.py
```

------------------------------------------------------------------------


## 🔍 核心实现

-   Agent执行流程：ReAct执行框架，解析Thought/Action并调度工具调用
-   工具系统：基于ToolRegistry实现模块化扩展（RAG / Memory）
-   RAG模块：基于向量数据库实现知识检索增强
-   执行控制：通过Prompt约束与状态控制，避免死循环与无效调用
-   稳定性优化：减少重复调用，控制推理步骤

------------------------------------------------------------------------

## 📌 技术栈

-   Python / FastAPI
-   ReAct （多步推理与工具调用）
-   LLM API
-   Qdrant（向量数据库）
-   Neo4j（图数据库）
-   Streamlit（前端）

------------------------------------------------------------------------

## 📎 项目说明

本项目重点在于：

-   Agent执行流程控制
-   多模块协同（Agent + RAG + Memory）
-   工程化实现（接口、结构、配置管理）

------------------------------------------------------------------------
